...
from lib.skin_loader import skin_dataset_train, skin_dataset_test
...
def load_data(opt):
    ...
    if opt.dataset in ['cifar10']:
        ...
        classes = {
            'mel': 0, 'nv': 1, 'bcc': 2, 'ak': 3, 'bkl': 4,
            'df': 5, 'vasc': 6, 'scc': 7
        }

        # dataset = {}
        # dataset['train'] = CIFAR10(root='./data', train=True, download=True, transform=transform)
        # dataset['train'] = CIFAR10(root='./data', train=True, download=True)
        ...
        dataloader = {}
        dataloader["train"] = skin_dataset_train()
        dataloader["test"] = skin_dataset_test()
        dataloader = {x: torch.utils.data.DataLoader(dataset=dataloader[x],
                                                     batch_size=opt.batchsize,
                                                     shuffle=shuffle[x],
                                                     num_workers=int(opt.workers),
                                                     drop_last=drop_last_batch[x]) for x in splits}
